...
- `rules/`: 복무 규정 근거와 판정 기준 요약을 관리합니다.
...
- `python scripts/attendance_harness.py`: 공개 샘플 기대 결과와 합성 회귀 케이스를 검증합니다.
...
## 테스트와 하네스 지침
현재 정식 `tests/` 디렉터리는 없습니다. 복무 규칙 변경 전후에는 반드시 `python scripts/attendance_harness.py`를 실행해 `sample_data/public_attendance_sample_expected.json`의 기대 행 번호와 비교합니다.

새 규칙을 추가할 때는 `rules/attendance_rules.md`에 규정 근거와 판정 기준을 먼저 적고, 공개 샘플 기대 행 번호를 `sample_data/public_attendance_sample_expected.json` 또는 하네스 합성 케이스에 고정합니다. 기관별 원문 규정과 실제 샘플은 로컬 비공개 자료로만 사용합니다.
